...
(define (letrec-declare expr)
  (mmap (lambda (x) (list (car x) '*unassigned*)) (letrec-bindings expr)))
(define (letrec-set expr)
  (mmap (lambda (x) (make-assignment (first x) (second x))) (letrec-bindings expr)))
(define (letrec->let expr)
  (let ((bind-variables (letrec-declare expr))
        (set-variables (letrec-set expr))
        (body (letrec-body expr)))
    (make-let bind-variables
              (make-begin (append set-variables body)))))
...
(define (m-eval exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((letrec? exp) (m-eval (letrec->let exp) env))
